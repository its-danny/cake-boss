...
describe('entities/config', () => {
  ...
  test('#setRoles', async done => {
    const config = new Config();
    const server = await createServer();
    const roleOne = { id: faker.random.uuid(), name: faker.internet.domainWord() } as Role;
    const roleTwo = { id: faker.random.uuid(), name: faker.internet.domainWord() } as Role;
    const message = await createMessage({ server, serverRoles: [roleOne, roleTwo] });
